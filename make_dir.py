...
def drop_file(event):
    ...
    file_path = event.data.strip('{}')  # Remove leading and trailing braces
    file_path_entry.insert(tk.END, file_path)

def drop_directory(event):
    directory_path_entry.delete(0, tk.END)
    directory_path = event.data.strip('{}')  # Remove leading and trailing braces
...
def create_dirs():
    ...
    start_row = int(start_row_entry.get()) - 1  # Subtract 1 because Python uses 0-indexing
...
start_row_frame.grid(row=2, column=0, columnspan=2, pady=10)
...
end_row_frame.grid(row=3, column=0, columnspan=2, pady=5)
...
start_button.grid(row=4, column=0, columnspan=4, pady=15)  # Add some vertical padding
